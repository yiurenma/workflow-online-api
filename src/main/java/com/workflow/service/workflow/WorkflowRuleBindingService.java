...
import static com.workflow.common.utils.AppConstant.workflowEntityLinkCache;
import static com.workflow.common.utils.AppConstant.workflowRuleBindingLinkingIdCache;
import static com.workflow.service.workflow.WorkflowDispatchService.ruleBindingsFullyMatch;

@Service
@Slf4j
public class WorkflowRuleBindingService {

    @Autowired
    CommonClientService commonClientService;
    @Autowired
    WorkflowRuleBindingRepository workflowRuleBindingRepository;
    @Autowired
    WorkflowEntityLinkRepository workflowEntityLinkRepository;
    @Autowired
    ...
    public CompletableFuture<JSONObject> executeLinkingOfRuleAndTypeWithAsync(
            ...
            List<WorkflowRuleBinding> ruleBindings) throws JsonProcessingException, ClassNotFoundException {
        return CompletableFuture.completedFuture(executeLinkingOfRuleAndType(runtimePayload, ruleBindings));
    }
    ...
    public JSONObject executeLinkingOfRuleAndType(
        ...
        if (ruleBindingsFullyMatch(runtimePayload, ruleBindings)) {
            WorkflowType wt = ruleBindings.get(0).getWorkflowType();
            ...
            if (DISPATCH.toString().equals(wt.getType())) {
                ...
                try {
                    ...
                    branchRecord.setWorkflowLinkingId(ruleBindings.get(0).getLinkingId());
                    ...
                    try {
                        ...
                        branchRecord.setOverallStatus(WorkflowRunStatus.SM_FAIL.toString());
                        branchRecord.setWorkflowResponseFromProvider(e.getMessage());
                    }
                } catch (Exception e) {
                    log.warn("Exception during outbound dispatch step : {}, exception details as : {}", branchRecord, e.getMessage());
                    log.warn("Exception stack", e);
                }
    ...
    @Cacheable(key = "#linkingId", value = workflowRuleBindingLinkingIdCache)
    public List<WorkflowRuleBinding> getWorkflowRuleBindingLinkingId(String linkingId) {
        return workflowRuleBindingRepository.getAllByLinkingId(linkingId);
    }

    @Cacheable(key = "#workflowEntitySettingId", value = workflowEntityLinkCache)
    public List<WorkflowEntityLink> getWorkflowEntityLink(Long workflowEntitySettingId) {
        return workflowEntityLinkRepository.findAllByWorkflowEntitySettingId(workflowEntitySettingId);
    }
}
